import { useDeleteTypeDocumentByIdMutation } from '../../context/Api/Common'
...
function ChangeStateTypeDocument () {
  const dispatch = useDispatch()
  const [deleteTypeDocument, { error, isLoading }] =
  useDeleteTypeDocumentByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteTypeDocument(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Tipo de documento cambio de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> este tipo de documento?</p>
...
export function ChangeStateButtonTypeDocument ({ typeDocument }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: typeDocument }))
  }
...
export default ChangeStateTypeDocument
