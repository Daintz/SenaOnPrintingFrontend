import { useDeleteGrammageCaliberByIdMutation } from '../../context/Api/Common'
...
function ChangeStateGrammageCaliber () {
  const dispatch = useDispatch()
  const [deleteGrammageCaliber, { error, isLoading }] =
  useDeleteGrammageCaliberByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteGrammageCaliber(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Gramaje y/o calibre cambio de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> este Gramaje y/o calibre?</p>
...
export function ChangeStateButtonGrammageCaliber ({ grammageCaliber }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: grammageCaliber }))
  }
...
export default ChangeStateGrammageCaliber
