import { useDeleteFinishByIdMutation } from '../../context/Api/Common'
...
function ChangeStateFinish ({ finish }) {
  const dispatch = useDispatch()
  const [deleteFinish, { error, isLoading }] =
  useDeleteFinishByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteFinish(changeStatusData.id)
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> este Acabado?</p>
...
export default ChangeStateFinish
