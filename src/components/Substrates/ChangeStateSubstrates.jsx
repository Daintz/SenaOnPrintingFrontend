import { useDeleteSubstratesByIdMutation } from '../../context/Api/Common'
...
function ChangeStateSubstrates () {
  const dispatch = useDispatch()
  const [deleteSubstrates, { error, isLoading }] =
  useDeleteSubstratesByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteSubstrates(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('este sustrato cambio de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> este sustrato?</p>
...
export function ChangeStateButtonSubstrates ({ substrates }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: substrates }))
  }
...
export default ChangeStateSubstrates
