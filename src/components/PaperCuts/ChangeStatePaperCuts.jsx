import { useDeletePaperCutsByIdMutation } from '../../context/Api/Common'
...
function ChangeStatePaperCuts () {
  const dispatch = useDispatch()
  const [deletePaperCuts, { error, isLoading }] =
  useDeletePaperCutsByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deletePaperCuts(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('este corte de papel cambio de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> este corte de papel?</p>
...
export default ChangeStatePaperCuts
