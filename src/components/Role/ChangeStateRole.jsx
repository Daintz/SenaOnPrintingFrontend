...
function ChangeStateRole () {
  ...
  const handleSubmit = async () => {
    await deleteRole(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Rol cambio de estado con exito')
  }
  ...
  return (
    <>
      <p className="text-lg">Para cambiar de estado este rol debes darle click a <b>aceptar</b>.</p>
...
export function ChangeStateButtonRole ({ role }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: role }))
  }

  return (
    <button type="button" onClick={ handleOpen } alt="Icono cambiar estado" title="Cambiar estado del rol">
      <svg
