import { useDeleteWarehauseTypeByIdMutation } from '../../context/Api/Common'
...
function ChangeStateWarehauseType ({ WarehauseType }) {
  const dispatch = useDispatch()
  const [deleteWarehauseType, { error, isLoading }] =
  useDeleteWarehauseTypeByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteWarehauseType(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Tipo de bodega  cambio de estado con exito')
  }
...
export function ChangeStateButtonWarehauseType ({ WarehauseType }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: WarehauseType }))
  }
...
export default ChangeStateWarehauseType
