import { useDeleteImpositionPlanchByIdMutation } from '../../context/Api/Common'
...
function ChangeStateImpositionPlanch ({ impositionPlanch }) {
  ...
  const [deleteImpositionPlanch, { error, isLoading }] =
  useDeleteImpositionPlanchByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteImpositionPlanch(changeStatusData.id)
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> esta imposición plancha?</p>
...
export function ChangeStateButtonImpositionPlanch ({ impositionPlanch }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: impositionPlanch }))
  }
...
export default ChangeStateImpositionPlanch
