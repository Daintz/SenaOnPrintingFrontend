...
function ChangeStateQuotationProviders ({ quotationProviders }) {
  const dispatch = useDispatch()
  const [deleteQuotationProviders, { error, isLoading }] =
  useDeleteQuotationProvidersByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteQuotationProviders(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Cotizacion a proveedores cambio de estado con exito')
  }
  const handle = async()=> {
    dispatch(closeModal())
  }
  ...
  return (
    ...
    <h1 className="text-4xl text-center font-bold">¿Estas seguro?</h1>
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> esta cotización?</p>
...
export function ChangeStateButtonQuotationProviders ({ quotationProviders }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: quotationProviders }))
  }
  return (
...
export default ChangeStateQuotationProviders
