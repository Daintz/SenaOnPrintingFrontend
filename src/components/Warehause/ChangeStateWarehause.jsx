import { useDeleteWarehauseByIdMutation } from '../../context/Api/Common'
...
function ChangeStateWarehause ({ warehause }) {
  const dispatch = useDispatch()
  const [deleteWarehause, { error, isLoading }] =
  useDeleteWarehauseByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteWarehause(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Bodega cambiada de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> esta bodega?</p>
...
export default ChangeStateWarehause
