import { useDeleteMachineByIdMutation } from '../../context/Api/Common'
...
function ChangeStatemachine ({ machine }) {
  const dispatch = useDispatch()
  const [deletemachine, { error, isLoading }] =
  useDeleteMachineByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deletemachine(changeStatusData.id)
...
export default ChangeStatemachine
