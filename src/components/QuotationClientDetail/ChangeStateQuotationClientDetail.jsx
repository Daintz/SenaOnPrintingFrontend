import { useDeleteQuotationClientDetailByIdMutation } from '../../context/Api/Common'
...
function ChangeStateQuotationClientDetail ({ quotationclientdetail }) {
  const dispatch = useDispatch()
  const [deleteQuotationClientDetail, { error, isLoading }] =
  useDeleteQuotationClientDetailByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteQuotationClientDetail(changeStatusData.id)
    if (isLoading) return <Spinner />
    if (error) return <Error type={error.status} message={error.error} />

    dispatch(changeAction())
    dispatch(closeModal())
    toast.success('Cotizacion cambia de estado con exito')
  }
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> esta cotizacion?</p>
...
export function ChangeStateButtonQuotationClientDetail ({ quotationclientdetail }) {
  const dispatch = useDispatch()
  const handleOpen = async () => {
    dispatch(setWidth({ width: '800px' }))
    dispatch(openModal({ title: 'Cambiar de estado' }))
    dispatch(setAction({ action: 'changing' }))
    dispatch(setChangeStatusData({ changeStatusData: quotationclientdetail }))
  }
...
export default ChangeStateQuotationClientDetail
