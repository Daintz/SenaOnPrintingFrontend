import { useDeleteUnitMesureByIdMutation } from '../../context/Api/Common'
...
function ChangeStateunit ({ unitMesure}) {
  const dispatch = useDispatch()
  const [deleteUnit, { error, isLoading }] =
  useDeleteUnitMesureByIdMutation()
  const { changeStatusData } = useSelector((state) => state.modal)

  const handleSubmit = async () => {
    await deleteUnit(changeStatusData.id)
  ...
  return (
    <>
      ...
      <p className="text-lg">¿Estas seguro de <b>cambiar de estado</b> esta Unidad?</p>
...
export default ChangeStateunit
