...
const ListLineature = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllLineaturesQuery()
  ...
  const columns = [
    { key: 'lineature', name: 'Lineatura' },
    { key: 'typePoint', name: 'Tipo de punto' },
  ...
  const rows = data
    ? data.map(lineature => ({
      id: lineature.id,
      lineature: lineature.lineature,
      typePoint: lineature.typePoint,
      statedAt: lineature.statedAt
    }))
    : []
            ...
            <tbody>
              {rows.map(lineature => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{lineature.typePoint}</td>
